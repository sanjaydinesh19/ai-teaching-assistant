...
export default function Worksheet() {
  ...
  const generateIdFor = (f: File) => {
    ...
    const safe = base.toLowerCase().replace(/[^a-z0-9_-]+/g, "-").slice(0, 40);
  ...
  return (
    <div className="p-6 max-w-3xl mx-auto text-gray-200">
      <h1 className="text-2xl font-bold mb-4 text-white">Worksheet Generator</h1>
      ...
      <form onSubmit={onSubmit} className="space-y-5">
        ...
        {mode === "per-set" && (
          <div className="border border-purple-500 rounded p-3 bg-[#1a1a25]">
            <p className="font-medium mb-2 text-white">Difficulty per Set</p>
            <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
              {Array.from({ length: numSets }).map((_, idx) => (
                <div key={idx} className="flex items-center gap-2">
                  <span className="text-sm text-gray-300 w-20">Set {idx + 1}</span>
                  <select
                    ...
                    className="bg-[#1a1a25] border border-purple-500 rounded p-2 flex-1 text-white"
                  >
                    {DIFFS.map(d => (
                      <option key={d} value={d}>{d}</option>
                    ))}
        ...
        <div className="border border-purple-500 rounded p-3 bg-[#1a1a25]">
          <p className="font-medium mb-2 text-white">Question Mix (optional)</p>
          <div className="grid grid-cols-1 md:grid-cols-3 gap-3">
            <label className="flex items-center gap-2">
              <span className="w-20">MCQ</span>
              <input type="number" min={0} value={mixMcq} onChange={e => setMixMcq(Number(e.target.value))} className="bg-[#1a1a25] border border-purple-500 rounded p-2 w-24 text-white" />
            </label>
            <label className="flex items-center gap-2">
              <span className="w-20">Short</span>
              <input type="number" min={0} value={mixShort} onChange={e => setMixShort(Number(e.target.value))} className="bg-[#1a1a25] border border-purple-500 rounded p-2 w-24 text-white" />
            </label>
            <label className="flex items-center gap-2">
              <span className="w-20">Diagram</span>
              <input type="number" min={0} value={mixDiagram} onChange={e => setMixDiagram(Number(e.target.value))} className="bg-[#1a1a25] border border-purple-500 rounded p-2 w-24 text-white" />
            </label>
          </div>
      ...
      {result && (
        <div className="mt-6 p-4 border border-purple-500 rounded bg-[#1a1a25] text-white">
          ...
          <ul className="space-y-2">
            {result.sets.map(s => (
              <li key={s.set_no} className="flex items-center justify-between bg-[#111] border border-purple-600 rounded p-2">
                <div>
